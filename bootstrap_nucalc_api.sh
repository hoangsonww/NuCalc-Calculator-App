...
# bootstrap_nucalc_api.sh
# —————————————————————————————————————————————————————————————————
# Creates a Sinatra + Mongoid + JWT backend for NuCalc Pro,
# then generates a gemspec and publish script.
# —————————————————————————————————————————————————————————————————
...
gem 'bcrypt',  '~> 3.1'
gem 'rack-cors', '~> 1.1'
gem 'pony',    '~> 2.18'    # for email notifications (optional)
EOF
...
# 6) Gem::Specification (gemspec)
cat > nu_calc_pro_api.gemspec <<'EOF'
...
echo "✅ Done! Your Sinatra API is in './ruby'."
echo "• Edit config/mongoid.yml & .env.example, then:"
echo "    cd ruby && bundle install"
echo "    foreman start    # or rackup"
echo "• To publish to GitHub Packages:"
echo "    cd ruby && ./publish.sh"
